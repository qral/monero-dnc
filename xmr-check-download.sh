...
if [[ -n "$1" ]]; then
 	...
 	`$curl $hashes_url`
 	if [[ "$?" -eq 0 ]]; then
 		echo "Downloading ~fluffy~ Binaryfate gpg key.."
 		`$curl $gpg_fluffy_url`
 		if [[ "$?" -eq 0 ]]; then
 			echo -e "Importing ~fluffy~ Binaryfate key..\n"
      # TODO!
...
# 
# 
# TODO 2: rm downloaded files (hashes + keys) at run otherwise it will download them as this:
# hashes.txt.1 
# binaryfate.asc.1
# 
